...
export function extractRecordText(data) {
  ...
  if (data.meta?.deployment) parts.push(`תעסוקה: ${data.meta.deployment}`);
  if (data.meta?.sector) parts.push(`גזרה: ${data.meta.sector}`);
  if (data.meta?.force) parts.push(`כוח: ${data.meta.force}`);
...
export const askInsights = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (filters?.deployment) query = query.where('meta.deployment', '==', filters.deployment);
