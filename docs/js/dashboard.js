...
async function fetchAllReviews(fromDate = null, maxDocs = 5000) {
  ...
  while (all.length < maxDocs) {
    // Increase limit to 1000 for fewer roundtrips
    const qParts = [collection(db, "reviews"), orderBy("createdAt", "desc"), limit(1000)];
    ...
    // Give UI thread a moment to breathe
    await new Promise(r => setTimeout(r, 0));
  }
  return all;
}

function aggregate(docs, { fromDate, toDateEnd, typeFilter, obsFilter, deploymentFilter }) {
  ...
  for (const { data } of docs) {
    ...
    if (deploymentFilter && (data?.meta?.deployment || '') !== deploymentFilter) continue;
...
async function initDashboard() {
  try {
    if (!globalLists) {
      ...
      console.log("[Dashboard] Lists loaded:", globalLists);
      
      const deploymentOptions = (globalLists.deployments || []).map(d => typeof d === 'object' ? d.name : d);
      populateSelect("deploymentFilter", deploymentOptions, val("deploymentFilter"), "הכל");
      
      updateDashboardSectors(); // Set initial SECTORS
...
function updateDashboardSectors() {
  if (!globalLists) return;
  const depName = val("deploymentFilter");
  const depObj = (globalLists.deployments || []).find(d => (typeof d === 'object' ? d.name : d) === depName);
  
  if (depObj && typeof depObj === 'object' && Array.isArray(depObj.sectors)) {
    const valid = depObj.sectors.filter(s => s && s.trim() !== "");
    if (valid.length > 0) {
      SECTORS = valid;
      return;
    }
  }
  // Fallback to all sectors from list or default
...
async function loadDashboard() {
  try {
    ...
    const deploymentFilterEl = document.getElementById("deploymentFilter");
    
    // Chunked aggregation to avoid blocking UI thread
    dashStatus.textContent = 'מנתח נתונים...';
    const agg = aggregate(docs, { 
      fromDate, 
      toDateEnd, 
      typeFilter: typeFilterEl?.value || '', 
      obsFilter: obsFilterEl?.value || '',
      deploymentFilter: deploymentFilterEl?.value || ''
    });
    ...
    try {
      ...
      if (container) {
        ...
        SECTORS.forEach((s, i) => {
          ...
          card.className = "card";
          card.innerHTML = `<h3>${s}</h3><div class="chartWrap"><canvas id="chart_sector_${i}"></canvas></div>`;
...
el('deploymentFilter')?.addEventListener('change', () => {
  updateDashboardSectors();
  // Optionally auto-reload, but usually users click 'Load'
});
