...
export async function initAiInsights() {
    ...
    askBtn.onclick = async () => {
        ...
        const deploymentVal = el("ai_deployment").value;
        const roleVal = el("ai_role")?.value;
        const forceVal = el("ai_force")?.value.trim();
        ...
        if (deploymentVal) { filters.deployment = deploymentVal; filterStrArr.push(`תעסוקה ${deploymentVal}`); }
    ...
    if (waExportBtn) {
        waExportBtn.onclick = () => {
            ...
            const textToShare = `🦉 *תובנות ינשוף AI*\n` +
                                `━━━━━━━━━━━━━━━\n\n` +
                                `❓ *השאלה שנשאלה:*\n${questionInput.value.trim()}\n\n` +
                                `💡 *תובנות ומסקנות:*\n${lastAnswer}\n\n` +
                                `🔍 *מידע שפולטר:* ${lastAppliedFiltersText}\n\n` +
                                `━━━━━━━━━━━━━━━\n` +
                                `_נוצר ע"י מערכת ינשוף_`;
    ...
    try {
        ...
        populateSelect("ai_sector", lists.sectors, "", "הכל");
        populateSelect("ai_role", lists.roles, "", "הכל");
        // For AI, we include ALL deployments as requested
        const allDeployments = (lists.deployments || []).map(d => typeof d === 'object' ? d.name : d);
        populateSelect("ai_deployment", allDeployments, "", "הכל");
    } catch (e) {
        console.error("AI: Failed to load lists", e);
    }
}
