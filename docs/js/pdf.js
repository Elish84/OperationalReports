...
export function buildWhatsappText(d) {
  ...
  if (d.gaps) lines.push("", `🧩 *פערים שעלו מהכוח:*`, d.gaps);
  if (d.keep?.length) { lines.push("", "💡 *נק׳ לשימור:*"); d.keep.slice(0,3).forEach((x) => lines.push(`• ${x}`)); }
  if (d.improve?.length) { lines.push("", "🛠️ *נק׳ לשיפור:*"); d.improve.slice(0,3).forEach((x) => lines.push(`• ${x}`)); }
  if (d.notes) lines.push("", "🗒️ *הערות:*", d.notes);
  return lines.join("\n");
}
...
export async function exportPdf(d, photos) {
  ...
  if (d.gaps) { addTitle("פערים שעלו מהכוח"); addParagraph(d.gaps); }
  if (d.keep?.length) { addTitle("נקודות לשימור"); d.keep.slice(0,3).forEach((t) => addLine(`• ${t}`)); }
  if (d.improve?.length) { addTitle("נקודות לשיפור"); d.improve.slice(0,3).forEach((t) => addLine(`• ${t}`)); }
  if (d.notes) { addTitle("הערות"); addParagraph(d.notes); }
