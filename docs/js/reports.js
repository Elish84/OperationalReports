...
function renderModalView(){
  ...
  const parts = [];
  parts.push(`<div class="small"><b>תעסוקה מבצעית:</b> ${h(m.deployment || '—')}</div>`);
...
function renderModalEdit(){
  ...
  modalBody.innerHTML = `
    ...
    <div class="row">
      <div><label>כוח/משימה</label><input id="e_force" value="${h(m.force || '')}" /></div>
      <div style="flex:2"><label>תעסוקה מבצעית</label>${selectHtml('e_deployment', (globalLists?.deployments || []).map(d => typeof d === 'object' ? d.name : d), m.deployment || '')}</div>
    </div>
...
function applyClientFilters(rows){
  ...
  return rows.filter(({data}) => {
    ...
    if (deployment && (d.meta?.deployment || '') !== deployment) return false;
  ...
  try {
    ...
    const payload = {
      ...
      meta: { 
        ...
        role: get('e_role')?.value || '', 
        sector: get('e_sector')?.value || '', 
        force: (get('e_force')?.value || '').trim(),
        deployment: get('e_deployment')?.value || ''
      },
...
function updateReportSectors() {
  if (!globalLists) return;
  const depName = el('filterDeployment')?.value;
  const depObj = (globalLists.deployments || []).find(d => (typeof d === 'object' ? d.name : d) === depName);
  
  let sectorsToShow = globalLists.sectors || [];
  if (depObj && typeof depObj === 'object' && Array.isArray(depObj.sectors)) {
    const valid = depObj.sectors.filter(s => s && s.trim() !== "");
    if (valid.length > 0) sectorsToShow = valid;
  }
  
  populateSelect('filterSector', sectorsToShow, el('filterSector')?.value, "הכל");
}
...
(async function init() {
  try {
    ...
    populateSelect('filterDeployment', deploymentOptions, "", "הכל");
    
    el('filterDeployment')?.addEventListener('change', updateReportSectors);
